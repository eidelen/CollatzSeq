...
TEST(MultiThreadCollatz, Suequence)
{
    std::shared_ptr<Collatz> c(new MultiThreadCollatz(2));
    auto seq = c->getSequence(3);
    auto prep_3_seq = getSeq3();
    ASSERT_EQ(seq.size(), prep_3_seq.size());
    ASSERT_TRUE(std::equal(seq.begin(), seq.end(), prep_3_seq.begin()));
...
TEST(MultiThreadCollatz, LongestSuequence)
{
    std::shared_ptr<Collatz> c(new MultiThreadCollatz(2));
    auto seq = c->getLongestSequence(1, 5);
    auto prep_3_seq = getSeq3();
...
TEST(MultiThreadCollatz, LongestSuequenceLong)
{
    std::shared_ptr<Collatz> c(new MultiThreadCollatz(2));
    auto seq = c->getLongestSequence(1, 100000);
    std::cout << "RandomAccess Collatz 1 Million: " << seq[0] << " , length = " << seq.size() << std::endl;
    ASSERT_TRUE(true);
}
